...
def main():
    ...
    data_flags = ('organmnist3d','nodulemnist3d','adrenalmnist3d','fracturemnist3d','vesselmnist3d','synapsemnist3d','tissuemnist', 'pathmnist', 'chestmnist', ' dermamnist', 'octmnist', 'pnemoniamnist', 'retinamnist', 'tissuemnist', 'organamnist', 'organcmnist', 'organsmnist')
    for data_flag in data_flags:  
      if data_flag in INFO:  
        info = INFO[data_flag]  # 获取与 data_flag 匹配的信息  
        # 在这里处理 info  
        task = info['task']
        if '3d' in data_flag:
        ...
        print(f"No information found for {data_flag}")

    n_channels = info['n_channels']
    n_classes = len(info['label'])
    #args_config['task'], args_config['in_channel'], args_config['num_classes'] = info['task'], info['n_channels'], len(info['label'])
    ...
    if args.dataset == "cifar10-dvs-tet":
        ...
        dataset_eval = DataClass(split='test', transform=test_transform, download=download)
    ...
    if args.dataset in dvs_utils.DVS_DATASET:
        ...
        loader_eval = torch.utils.data.DataLoader(
          dataset=dataset_eval, batch_size=2*args.batch_size, shuffle=False
        )
    else:
        ...
        loader_eval = create_loader(
            dataset_eval,
            input_size=data_config["input_size"],
            batch_size=2*args.val_batch_size,
            is_training=False,
            use_prefetcher=args.prefetcher,
...
def train_one_epoch(
    ...
    device = next(model.parameters()).device  # 获取网络参数所在的设备  
